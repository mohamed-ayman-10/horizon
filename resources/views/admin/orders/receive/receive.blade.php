...
@section('main-header', 'Receive')
@section('header', 'Orders')
@section('title_header', 'Receive')
@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Add Receive</h3>
        </div>

        <form action="{{ route('admin.order.postReceive') }}" method="post">
            <div class="card-body">
                @csrf
            ...
            <div class="table-responsive">
                @can('order table')
                    <table class="table table-bordered text-center text-nowrap border-bottom" id="basic-datatable">
                        ...
                        <tbody>
                            @foreach ($receive as $index => $item)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ $item->id }}</td>
                                    <td>
                                        <a
                                            href="{{ route('admin.admins.edit', $item->sendOrder->admin->id) }}">{{ $item->sendOrder->admin->name }}</a>
                                    </td>
                                    @if ($item->created_at)
